<tr data-task-id="{{ $task->id }}" class="transition-colors duration-200 hover:bg-slate-50">
    <td class="px-4 py-3">
        <form method="POST" action="{{ route('tasks.update', $task) }}" data-ajax-row-form="task" data-record-id="{{ $task->id }}" data-record-action="toggle">
            ...
            <button type="submit" data-loading-label="Updating..." class="inline-flex h-6 w-6 items-center justify-center rounded-md border transition-colors duration-200 {{ $task->status === 'completed' ? 'bg-green-600' : 'bg-white' }}">
